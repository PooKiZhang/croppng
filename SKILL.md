...
- 只有在当前环境完全没有可用的图片生成/编辑能力，或完全没有任何方式把 crop 作为图片参考传入时，才停止并说明能力缺口。
...
- 如果当前工具只能接收纯文本、无法接收或引用真实 crop 图片，停止并说明“缺少参考图交接能力”；不要改成只用文字描述生成，也不要要求用户提供外部 API。
...
- 如果经过实际检查，当前环境完全没有任何方式把 crop 作为图片上下文、附件、文件引用或编辑输入传给可用图片生成工具，必须停下说明“无法执行参考图重绘”，不要继续做本地抠图替代，也不要改为索要外部 API。
...
3. AI 重绘
   ...
   - 如果完全无法把 crop 作为图片参考传入，停止并报告，不能进入最终 PNG 生成
   - 必须把 crop 的真实图片内容作为参考输入；如果做不到，停止并报告缺口
