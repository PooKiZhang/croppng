...
description: 从参考图中切出独立元素到本地，再把每个 crop 作为图像参考交给 Codex 当前可用的内置图片生成/编辑能力重绘，最后只从 AI 重绘图输出透明 PNG。适用于 UI 还原、贴纸拆解、素材提取、插画资产整理；严禁把本地 crop 抠透明后当最终图交付，也不要向用户索要 image-2 API。
---
...
1. 切图到本地（按独立元素）
2. 把每个 crop 作为参考图交给 Codex 当前可用的内置图片生成/编辑能力重绘
...
- 本 skill 中的“AI 重绘”“图片生成”“image-2”都指 Codex 当前环境已经提供的图片生成/图片编辑能力，不等于要求用户提供 OpenAI API key、image-2 API、SDK 或环境变量。
- 优先使用当前对话/应用里已经可调用的内置生图工具；不要因为看到 `image-2` 这个词就停下来要求用户配置外部 API。
- 如果存在多个可用工具，选择能把 crop 图像本身作为参考输入的工具。工具名可以是 image generation、image editing、imagegen、内置图片生成等，不要求固定叫 `image-2`。
- 只有在当前环境完全没有可用的图片生成/编辑能力，或可用工具无法接收 crop 作为图像参考时，才停止并说明能力缺口。
...
- 如果当前可用的图片生成工具 schema 只有文本 prompt、没有图片/附件/reference image 输入能力，必须停下说明“无法执行参考图重绘”，不要继续做本地抠图替代，也不要改为索要 image-2 API。
...
另一个常见错误是：把“image-2”理解成外部 API 要求，于是要求用户提供 API key 或让用户安装 SDK。不要这样做。Codex 环境里若已有内置图片生成/编辑能力，应直接使用该能力完成重绘。

遇到“把图片切出来做成透明 PNG”“批量抠图”“生成独立素材”这类请求时，先问自己：

- 我有没有把每个 crop 图像本身传给可用的内置图片生成/编辑工具作为参考？
...
如果任何答案是否定的，不能交付最终 PNG。此时应停在当前阶段并说明：已完成本地 crop，但尚未完成 AI 参考图重绘。
...
- 不要让用户提供 image-2 API；除非用户明确要求用外部 API，否则使用 Codex 已有的内置生图能力。
...
3. AI 重绘
   ...
   - 不要要求用户提供 image-2 API key；`image-2` 只是可能的模型/能力代称，不是本 skill 的外部依赖
